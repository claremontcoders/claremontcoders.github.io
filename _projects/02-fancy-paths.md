...
3. What are some differences between the functions you listed in [1] and [2]? Does it make sense to you that some of these functions accept an input and others do not---why or why not?
...
**BONUS:** The functions `square()` and `triangle()` look similar in that they are alternating calls to `turtle.foward()` and `turtle.left()`. Is there a way to define a function `polygon(steps, angle)` that produces a square with one set of inputs and triangles with another set?
